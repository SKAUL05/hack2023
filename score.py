    ...
    with open('emissions.csv', mode='r') as file:
        ...
        if os.path.isfile("emissions.json"):
            with open("emissions.json", "r") as f:
                data = json.load(f)
                print(
                    f'{Fore.GREEN}New Carbon footprint: {round(data["CARBON_FOOTPRINT"] - emissions, 5)} kg CO2'
                )
                print(
                    f'{Fore.GREEN}New Green Energy Score: {round(data["ENERGY_SCORE"] - green_score, 5)} kW'
                )
